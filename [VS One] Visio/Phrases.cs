...
namespace _VS_One__Visio
{
    ...
    {
        public bool onlyElaboratedPhrases = false;

        public Phrases(bool oep)
        {
            InitializeComponent();
            onlyElaboratedPhrases = oep;
        }
        ...
        public void setListElements(bool ValidationNeeded = true)
        {
            ...
            List<PhrasesArray> listOfPhrases = phrasesFunctions.getAllPhrases(shapes, ValidationNeeded, onlyElaboratedPhrases);
            ...
            foreach (PhrasesArray phrase in listOfPhrases)
            {
                if (!dictShape.ContainsKey(phrase.phrase))
                {
                    ...
                    item.SubItems.Add(phrase.nameSpace);
        ...
        {
            string[] splitString = (checkBox2.Checked)? mainText.Split(comboBox1.Text.ToCharArray()) : Regex.Split(mainText, @comboBox1.Text);
            foreach (string str in splitString)
            {
                if (!String.IsNullOrEmpty(str))
                {
                    setTextInExcelCells(worksheet, index + subIndex, intent, str, nameSpace);
                    subIndex++;
                }
            }
            subIndex--;
        ...
        {
            string textInCell = String.Format("{0} {1}", intent, mainText);
        ...
        public void checkBlocks()
        {
            ...
            List<PhrasesArray> listOfPhrases = phrasesFunctions.getAllPhrases(Globals.ThisAddIn.Application.ActivePage.Shapes, true, onlyElaboratedPhrases);
            ...
            foreach(Visio.Shape shp in Globals.ThisAddIn.Application.ActivePage.Shapes)
            {
                ...
                {
                    if (!list.Contains(shp) && !validateList.Contains(shp)) list.Add(shp);
                }
            }
            ...
            {
                if (!dictShape.ContainsKey(shp.Text) && !textAnalysis.containsBlock(shp.Text))
                {
