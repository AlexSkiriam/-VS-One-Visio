...
{
    public partial class NewTextEditorForm : Form
    {
        int lastCaretPos = 0;
        ...
        private void Scintilla1_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                ...
                MenuItem collectAllExp = new MenuItem("Собрать список expressions");
                collectAllExp.Click += delegate (object sender2, EventArgs e2) { collectUsableExpressions(); };
                menu.MenuItems.Add(collectAllExp);

                MenuItem collectAllDict = new MenuItem("Собрать список используемых словарей");
                collectAllDict.Click += delegate (object sender2, EventArgs e2) { collectUsableDicts(); };
                menu.MenuItems.Add(collectAllDict);

                MenuItem collectByTokens = new MenuItem("Получить словари по токенам");
                collectByTokens.Click += delegate (object sender2, EventArgs e2) { collectDictByTokens(); };
                menu.MenuItems.Add(collectByTokens);
                ...
                ((Scintilla)sender).ContextMenu = menu;
                menu.Show(scintilla1, e.Location);
            }
        }

        public void collectDictByTokens()
        {
            WorkWithPhrases withPhrases = new WorkWithPhrases(scintilla1.Text);
            withPhrases.Show();
        }

        public void collectUsableExpressions()
        {
            List<string> expressions = Regex.Matches(scintilla1.Text, @"\s<([^> < "" ""]+)>\s").Cast<Match>().Where(x => !x.Value.Contains("=")).Select(match => match.Groups[1].Value).ToList();
            expressions = expressions.Distinct().ToList();
            BasicPhraseBuldier basic = new BasicPhraseBuldier(String.Join("\n", expressions));
            basic.ShowDialog();
        }

        public void collectUsableDicts()
        {
            try
            {
                List<string> usableDict = new List<string>();
                List<string> tokens = new List<string>() { "phrases_exact", "phrases_parts", "phrases_parts_exclude", "phrases_exact_exclude" };
                var json = JToken.Parse(parser.parseConverting(scintilla1.Text))["states"].Where(x => x.SelectToken("phrases_conditions") != null).Cast<JToken>();
                foreach (var tok in json)
                {
                    var newTok = tok.SelectToken("phrases_conditions");
                    if (newTok != null)
                    {
                        foreach (var t in newTok)
                        {
                            tokens.ForEach(x =>
                            {
                                var innerTok = t.SelectToken(x);
                                if (innerTok != null)
                                {
                                    if (innerTok.Type == JTokenType.Array) innerTok.ToList().ForEach(y => { usableDict.Add(y.ToString()); });
                                    else if (innerTok.Type == JTokenType.String) usableDict.Add(innerTok.ToString());
                                }
                            });
                        }
                    }
                }
                var jsonNew = JToken.Parse(parser.parseConverting(scintilla1.Text))["global_listening_states"];
                foreach (var tok in jsonNew)
                {
                    var newTok = tok.SelectToken("phrases_conditions");
                    if(newTok != null)
                    {
                ...
                usableDict = usableDict.Distinct().ToList();
                BasicPhraseBuldier basic = new BasicPhraseBuldier(String.Join("\n", usableDict));
                basic.ShowDialog();
            }
            catch
        ...
        internal class DynamicCollection : IEnumerable<AutocompleteItem>
        {
            ...
            private IEnumerable<AutocompleteItem> BuildList()
            {
                ...
                foreach (Match m in Regex.Matches(tb.Text, @"(store|parameters|static_parameters|output)\.\w+"))
                    words[m.Value] = m.Value;
